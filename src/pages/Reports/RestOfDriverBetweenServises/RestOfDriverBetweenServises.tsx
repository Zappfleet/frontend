...
import useReportRestOfServices from '../../../hooks/data/reports/useReportRestOfServices.js';
...
export default function RestOfDriverBetweenServises({ handleBackClick, title }: any) {
    ...
    const { missionList, state, refreshData } = useReportRestOfServices("DONE", persianDateToGregorian(fromDate), persianDateToGregorian(toDate));
    ...
    const fetchData = () => {
        ...
        missionList.data && missionList.data.map((item: any) => {
            ...
            record = {
                ...
                name: `${item.name}`,
                startCurrentService: convertToJalaliDateTiem(item.startCurrentService),
                endBeforeService: convertToJalaliDateTiem(item.endBeforeService),
                RestOfServices: item.RestOfServices && (item.RestOfServices).toString()
            }
