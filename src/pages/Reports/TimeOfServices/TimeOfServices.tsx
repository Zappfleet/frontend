...
export default function TimeOfServices({ handleBackClick, title }: any) {
    ...
    const fetchData = () => {
        let k: any = []
        missionList.data && missionList.data.map((item: any, index: any) => {
            ...
            const differenceInSeconds = !isNaN(date1 - date2) ? Math.round((date1 - date2) / 1000) : '---'  //second
            index === 0 && console.log(79, differenceInSeconds);
            ...
            record = {
                ...
                name: `${item.name}`,
                startDate: item.mission_start,//convertToJalaliDateTiem(item.mission_start),
                endDate: item.mission_end,//convertToJalaliDateTiem(item.mission_end),
                time: differenceInSeconds === '---' ? '---' : secondsToHMS(differenceInSeconds)
            }
