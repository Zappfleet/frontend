...
const AreaManager = () => {
    ...
    const { refreshData: fetchUserData }: any = usePermittedUsers({ permissions: ["SERVICE.ORG.DISPATCH"] });
    ...
    return (
        <div className='absolute left-0 right-0 bottom-0 top-18'>
            ...
            <img
                className='absolute z-999999 left-2/4 -translate-x-2/4 bottom-2/4 w-8 h-12'
                src={MarkerRed} />

            <button onClick={handle_toggleDisplayList} className='fixed right-2 lg:right-75 top-20 shadow bg-white active:saturate-50 w-14 h-14 p-3 flex justify-center items-center text-white p-3 rounded-full'>
                <BiMenu className={"text-graydark"} size={30} />
            </button>
            ...
            <div className={classNames(
                'z-999999 duration-300 shadow fixed left-0 w-60 bg-white bottom-2 top-20 rounded-r-2xl px-1 flex flex-col',
                {
