...
const SignUp = () => {
  ...
  const validationRules = combineValidationRules(
    authInfo?.org?.additionalSignUpFields
  );

  const { validate, errors }: any = useValidation(validationRules);

  const [digitInputValue, onDigitInputChange] = useState<string>('');
  ...
  const regFormRef = useRef<any>();
  const verifyFormRef = useRef<any>();
  ...
  const handle_submitCodeVerify = (e: any) => {
    e.preventDefault();

    if (digitInputValue.length < 6) return;
    const formData = {
      username: extraDataRef.current.username,
      code: digitInputValue,
    };
    getApiClient()
      .verifyRegister(formData)
      .then(({ data }) => {
        storeTokens(data);
  ...
  const handle_submitRegsiter = (e: any) => {
    e.preventDefault();

    const formData = readFormInputs(regFormRef);
    ...
    setLoading(true);
    const body = { ...formData };
    body.key = signupMethodRef.current.value;
    ...
    getApiClient()
      ...
      .then(({ data }) => {
        ...
        setUi({ isLoading: false, showForm: FORM_VERIFY });
      })
      .catch((e) => {
        NotificationController.showError(e.response?.data?.error || e.message);
        setLoading(false);
          ...
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 ">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <h2 className="mb-9 text-center text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
                ثبت نام در سامانه ی مدیریت ناوگان
              </h2>

              <form
                className={classNames({ hidden: ui.showForm != FORM_VERIFY })}
                ref={verifyFormRef}
              >
                <div className="mb-4">
                  ...
                  <div className="relative flex items-center justify-center text-center">
                    <div className="digit-input-group ltr flex w-full justify-between">
                      <input
                ...
                <div className="mb-5">
                  <LoaderButton
                    ...
                    onClick={handle_submitCodeVerify}
                    className={
              ...
              <select
                className={classNames('mb-4 w-full p-2', {
                  hidden: ui.showForm != FORM_REG,
                })}
                ref={signupMethodRef}
              ...
              <form
                className={classNames({ hidden: ui.showForm != FORM_REG })}
                ref={regFormRef}
              >
                ...
                {selectedSignupMethodData.ignore_password != true && (
                  <>
                    <div className="mb-4">
                      <label className="mb-2.5 block font-medium text-black dark:text-white">
                        رمز ورود
                    ...
                    <div className="mb-4">
                      <label className="mb-2.5 block font-medium text-black dark:text-white">
                        تکرار رمز ورود
                        <ErrorMessage message={errors.password_confirm} />
                      </label>
                      <div className="relative flex items-center justify-center">
                        <input
                          type="password"
