...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ...
      className={`absolute right-0 top-0 flex shadow-lg h-screen w-72.5 flex-col overflow-y-hidden bg-white duration-100 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 pt-12 lg:pt-10 ${sidebarOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
    >
      {/* <!-- SIDEBAR HEADER --> */}
            ...
            <div>
              {
                navigationMenu?.map((menuItem: NavigationMenuItem) => {
                  if (menuItem.is_title) return <h3 key={menuItem.key} className="mb-4 ml-4 text-sm font-semibold ">{menuItem.label}</h3>
                  if (menuItem.sub_menu == null) return <li key={menuItem.key}>
                    <NavLink
                      onClick={() => setSidebarOpen(false)}
                      to={menuItem.href as string}
                      className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium duration-300 ease-in-out hover:text-primary hover:bg-gray-2 dark:hover:bg-meta-4 ${pathname.includes('calendar') && 'bg-graydark dark:bg-meta-4'}`}
                    >
                      {menuItem.icon}
                      {menuItem.label}
                    </NavLink>
                  ...
                  return <ul key={menuItem.key} className="mb-6 flex flex-col gap-1.5">
                    <SidebarLinkGroup
                      activeCondition={
