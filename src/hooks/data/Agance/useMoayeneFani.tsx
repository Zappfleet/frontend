...
export default function useMoayeneFani(action_name: any, item: any) {
      ...
      //  console.log(300, item, action_name);
        switch (action_name) {
            case 'select':
                ...
                getApiClient().select_MoayeneFani()
                    .then(resp => {
                        setResult(resp)
                        setType('select')
                    })
            ...
            case "insert":
                if (item !== null) {
                    setState({ ...state, in_progress: true })
                    getApiClient().insert_MoayeneFani(item)
                        .then(resp => {
                            setResult(resp)
                            setType('insert')
                        })
            ...
            case 'update':
                if (item !== null) {
                    setState({ ...state, in_progress: true })
                    getApiClient().update_MoayeneFani(item)
                        .then(resp => {
                            setResult(resp)
                            setType('update')
                        })
            ...
            case 'delete':
                setState({ ...state, in_progress: true })
                getApiClient().delete_MoayeneFani(item._id)
                    .then(resp => {
                        setResult(resp)
                        setType('delete')
                    })
