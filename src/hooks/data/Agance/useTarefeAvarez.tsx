...
export default function useTarefeAvarez(action_name: any, item: any) {
    ...
    function refreshData() {
        ...
        switch (action_name) {
            case 'select':
                ...
                getApiClient().select_TarefeAvarez()
                    .then(resp => {
                        setResult(resp)
                        setType('select')
                    })
            ...
            case "insert":
                if (item !== null) {
                    setState({ ...state, in_progress: true })
                    getApiClient().insert_TarefeAvarez(item)
                        .then(resp => {
                            setResult(resp)
                            setType('insert')
                        })
            ...
            case 'update':
                if (item !== null) {
                    setState({ ...state, in_progress: true })
                    getApiClient().update_TarefeAvarez(item)
                        .then(resp => {
                            setResult(resp)
                            setType('update')
                        })
            ...
            case 'delete':
                setState({ ...state, in_progress: true })
                getApiClient().delete_TarefeAvarez(item._id)
                    .then(resp => {
                        setResult(resp)
                        setType('delete')
                    })
