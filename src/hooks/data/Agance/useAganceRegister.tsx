...
export default function useAganceRegister(action_name: any, item: any) {
      ...
      //  console.log(300, item, action_name);
        switch (action_name) {
            ...
            case "insert":
                if (item !== null) {
                    setState({ ...state, in_progress: true })
                    getApiClient().insert_Agance(item)
                        .then(resp => {
                            setResult(resp)
                            setType('insert')
                        })
            ...
            case 'update':
                if (item !== null) {
                    setState({ ...state, in_progress: true })
                    getApiClient().update_Agance(item)
                        .then(resp => {
                            setResult(resp)
                            setType('update')
                        })
            ...
            case 'delete':
                setState({ ...state, in_progress: true })
                getApiClient().delete_Agance(item._id)
                    .then(resp => {
                        setResult(resp)
                        setType('delete')
                    })
