...
function ModularForm({ fields, onInputChange, mode, formState, clsssName1 }: any) {
  //console.log(800, clsssName1, clsssName1['proj_codedd']);

  return (
    <>
      {fields?.map((moduleProps: any) => {
        // console.log(760, formState[moduleProps.key], moduleProps.key);

        return renderUi(
          <ModularInput
            {...moduleProps}
            className1={clsssName1 && clsssName1[moduleProps.key] && clsssName1[moduleProps.key] === false ? true : false}
